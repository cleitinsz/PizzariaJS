        ...
        cs('.pizzaInfo--size').forEach((size, sizeIndex) => {
            size.addEventListener('click', (e)=>{
                ...
                if(sizeIndex == 0){
                    ...
                    c('.pizzaInfo--actualPrice').innerHTML = `R$ ${pizzaJson[key].price[1].toFixed(2)}`;
                } if(sizeIndex == 2){
                    c('.pizzaInfo--actualPrice').innerHTML = `R$ ${pizzaJson[key].price[2].toFixed(2)}`;
                }
    ...
    if(cart.length > 0){
        ...
        for(let i in cart){
            ...
            switch(cart[i].size){
                case 0:
                    pizzaSizeName = "P";
                    preço = pizzaItem.price[0];
                    break;
                case 1:
                    pizzaSizeName = "M";
                    preço = pizzaItem.price[1];
                    break;
                case 2:
                    pizzaSizeName = "G";
                    preço = pizzaItem.price[2];
                    break;
                    
            }
            subtotal += preço * cart[i].qt;
